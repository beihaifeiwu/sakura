...
public class Contouring {
    ...
    private static short findTriangleSide(TriMarkers currentTriangle, double isoValue,
        ...
        if (sideException != 0
                && computeSplitPosition(currentTriangle.getM1(), currentTriangle.getM2(),
                isoValue, currentTriangle.p0, currentTriangle.p1,
                splitPosition)) {
            return 0;
        } else if (sideException != 1
                && computeSplitPosition(currentTriangle.getM2(), currentTriangle.getM3(),
                isoValue, currentTriangle.p1, currentTriangle.p2,
                splitPosition)) {
            return 1;
        } else if (sideException != 2
                && computeSplitPosition(currentTriangle.getM3(), currentTriangle.getM1(),
    ...
    public static boolean splitInterval(double beginIncluded, double endExcluded,
        ...
        short vertIso1Start = -1;
        short vertIso1Stop = -1; // for beginIncluded -
        // Vertice of the
        // triangle where the
        // split Origin and
        // destination will be
        // done
        short vertIso2Start = -1;
        short vertIso2Stop = -1; // for endExcluded -
        // Vertice of the triangle where the split Origin and destination will be done
        short sideIso1Start = -1;
        short sideIso1Stop = -1; // for beginIncluded - Side
        // of the triangle where
        // the split Origin and
        // destination will be
        // done
        short sideIso2Start = -1;
        short sideIso2Stop = -1; // for endExcluded - Side
        // of the triangle where
        // the split Origin and
        // destination will be
        // done
        ...
        if (isoEqual(currentTriangle.getM1(), beginIncluded)) {
            vertIso1Start = 0;
        }
        if (isoEqual(currentTriangle.getM2(), beginIncluded)) {
            if (vertIso1Start == -1) {
                vertIso1Start = 1;
            } else {
                vertIso1Stop = 1;
        ...
        if (isoEqual(currentTriangle.getM3(), beginIncluded)) {
            if (vertIso1Start == -1) {
                vertIso1Start = 2;
            } else {
                vertIso1Stop = 2;
        ...
        // Find if we include some vertices
        if (isoEqual(currentTriangle.getM1(), endExcluded)) {
            vertIso2Start = 0;
        }
        if (isoEqual(currentTriangle.getM2(), endExcluded)) {
            if (vertIso2Start == -1) {
                vertIso2Start = 1;
            } else {
                vertIso2Stop = 1;
        ...
        if (isoEqual(currentTriangle.getM3(), endExcluded)) {
            if (vertIso2Start == -1) {
                vertIso2Start = 2;
            } else {
                vertIso2Stop = 2;
        ...
        if ((sideIso1Start == -1 && sideIso2Start == -1)
            ...
            if ((vertIso1Start != -1 && currentTriangle.getMaxMarker(vertIso1Start) < beginIncluded)
                ...
                || (vertIso2Start != -1 && vertIso2Stop != -1 && vertIso1Start != -1)) {
            // Covered totally by the range
            intervalTriangles.add(currentTriangle);
            return true;
            ...
            if (vertIso1Start != -1) {
                ...
                short vertOutside = -1;
                short vertInside = -1;
                if (currentTriangle.getM1() < beginIncluded) {
                    vertOutside = 0;
                    if (vertIso1Start == 1) {
                        vertInside = 2;
                    } else {
                ...
                } else if (currentTriangle.getM2() < beginIncluded) {
                    vertOutside = 1;
                    if (vertIso1Start == 0) {
                        vertInside = 2;
                    } else {
                ...
                } else if (currentTriangle.getM3() < beginIncluded) {
                    vertOutside = 2;
                    if (vertIso1Start == 0) {
                        vertInside = 1;
                    } else {
                ...
                outsideTriangles.add(new TriMarkers(currentTriangle.getVertice(vertIso1Start),
                        currentTriangle.getVertice(vertOutside), posIso1Start, beginIncluded,
                        currentTriangle.getMarker(vertOutside), beginIncluded));
                intervalTriangles.add(new TriMarkers(currentTriangle.getVertice(vertIso1Start),
                        currentTriangle.getVertice(vertInside), posIso1Start, beginIncluded,
                ...
                if (isoEqual(currentTriangle.getM1(), maxMarker)) {
                    vertOutside = 0;
                    if (vertIso2Start == 1) {
                        vertInside = 2;
                    } else {
                ...
                } else if (isoEqual(currentTriangle.getM2(), maxMarker)) {
                    vertOutside = 1;
                    if (vertIso2Start == 0) {
                        vertInside = 2;
                    } else {
