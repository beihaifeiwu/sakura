...
public class SpiPluginLoader<T> {
    ...
    private final Lazy<Map<String, Class<? extends T>>> cachedClasses = Lazy.of(this::loadPluginClasses);
    ...
    private Map<String, Class<? extends T>> loadPluginClasses() {
        val file = PLUGINS_DIRECTORY + type.getName();
        val urls = CLS.getResources(file);
        val classes = new HashMap<String, Class<? extends T>>();
        for (URL url : urls) {
